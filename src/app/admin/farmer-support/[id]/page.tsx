...
export default function EditSchemePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchScheme() {
      try {
        const res = await fetch(`/api/admin/farmer-schemes/${params.id}`)
        const data = await res.json()
        if (!res.ok) throw new Error(data.error || 'Failed to fetch scheme')
        setForm(data.item)
      } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6 text-center">
        <div className="bg-white p-10 rounded-3xl shadow-xl border border-gray-100 max-w-md">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Scheme Updated!</h1>
          <p className="text-gray-600 mb-8">The financial support scheme has been updated successfully.</p>
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Edit Farmer Scheme</h1>
        </div>
