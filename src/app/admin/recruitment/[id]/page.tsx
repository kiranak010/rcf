...
export default function RecruitmentForm() {
      ...
      fetch(`/api/recruitment/${params.id}`)
        ...
        .then((data) => {
          setForm({
            ...
            closingDate: data.item?.closingDate ? new Date(data.item.closingDate).toISOString().split('T')[0] : '',
          })
          setLoading(false)
        })
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    await fetch(`/api/recruitment${isEdit ? `/${params.id}` : ''}`, {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{isEdit ? 'Edit' : 'New'} Job Posting</h1>
      </div>
