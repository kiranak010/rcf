...
export default function AnnouncementForm() {
      ...
      fetch(`/api/announcements/${params.id}`)
        ...
        .then((data) => {
          setForm({
            ...
            status: data.item?.status || 'published',
          })
          setLoading(false)
        })
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    await fetch(`/api/announcements${isEdit ? `/${params.id}` : ''}`, {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{isEdit ? 'Edit' : 'New'} Announcement</h1>
      </div>
