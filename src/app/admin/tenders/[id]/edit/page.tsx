...
export default function EditTenderPage() {
  ...
  const handleSubmit = async (status: string) => {
    setSaving(true)
    await fetch(`/api/tenders/${params.id}`, {
      method: 'PUT',
  ...
  const handleDelete = async () => {
    await fetch(`/api/tenders/${params.id}`, { method: 'DELETE' })
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Update tender details</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => handleSubmit('upcoming')} disabled={saving}>Save Draft</Button>
          <Button onClick={() => handleSubmit('active')} disabled={saving}>Publish</Button>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Input id="tenderNumber" value={form.tenderNumber} onChange={(e) => setForm({ ...form, tenderNumber: e.target.value })} required />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="tenderValue" value={form.tenderValue} onChange={(e) => setForm({ ...form, tenderValue: e.target.value })} />
          </div>
          <div>
            <Label htmlFor="contactDetails">Contact Details</Label>
            <Textarea id="contactDetails" value={form.contactDetails} onChange={(e) => setForm({ ...form, contactDetails: e.target.value })} rows={3} />
          </div>
          <div>
        ...
        <CardFooter className="flex justify-between">
          <Button variant="outline" onClick={() => router.push('/admin/tenders')}>Cancel</Button>
          <div className="flex gap-2">
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogHeader>
          ...
          <DialogDescription>Are you sure you want to delete this tender? This action cannot be undone.</DialogDescription>
        </DialogHeader>
        <DialogFooter>
